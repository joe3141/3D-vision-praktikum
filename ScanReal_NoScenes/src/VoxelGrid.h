...
struct Voxel {
	Voxel() {
		...
		panoptic_weight = std::numeric_limits<float>::min();
		color_weight = std::numeric_limits<float>::min();
		panoptic_id = 0;
		isInstance = 0;
	}
	...
	float           color_weight;
	float			panoptic_weight;
	unsigned int    panoptic_id;
	uchar           isInstance;
};
...
public:
	...
	VoxelGrid(const vec3l& dim, const mat4f& worldToGrid, float voxelSize, const OBB3f& gridSceneBounds, float depthMin, float depthMax) : Grid3(dim.x, dim.y, dim.z) {
		...
		m_trunaction = m_voxelSize * 4.0f;
		m_truncationScale = m_voxelSize;
		m_weightUpdate = 1;

		instanceIds.reserve(41000);
...
//	void integrate(const mat4f& intrinsic, const mat4f& cameraToWorld, const DepthImage32& depthImage, const BaseImage<unsigned char>& label, const BaseImage<unsigned char>& instance, bool debugOut = false);
	void VoxelGrid::integrate(const mat4f& intrinsic, const mat4f& cameraToWorld, const DepthImage32& depthImage, const BaseImage<unsigned char>& label, const BaseImage<unsigned char>& instance, const BaseImage<unsigned char>& instance_gt,
		BaseImage<unsigned char>& panoptic, BaseImage<unsigned char>& isInstance, BaseImage<vec3f>& colorImage, bool debugOut = false);
	void generateReferenceLabel(const mat4f& intrinsic, const mat4f& cameraToWorld, const DepthImage32& depthImage, BaseImage<int>& referencePanoptic, BaseImage<unsigned char>& referenceIsInstance) const;
	void createConsistentLabel(BaseImage<unsigned char>& liveLabel, BaseImage<unsigned char>& liveIsInstance, BaseImage<int>& referenceLabel, BaseImage<unsigned char>& referenceIsInstance, BaseImage<int>& consistentLabel,
		BaseImage<unsigned char>& consistentIsInstance);
	Voxel getVoxelFromDepthPixel(float d, int u, int v, const mat4f& intrinsic, const mat4f& cameraToWorld, Voxel& voxel);
	...
	vec2ui countOccupancyAABB(const bbox3f& aabb, unsigned int weightThresh, float sdfThresh, unsigned short instanceId, MaskType& mask) const;
	...
	TriMeshf computeLabelMesh(float sdfThresh) const;
	TriMeshf computeInstanceMesh(float sdfThresh) const;
...
private:
	...
	std::unordered_map<int, int> instanceIds; // id -> count
