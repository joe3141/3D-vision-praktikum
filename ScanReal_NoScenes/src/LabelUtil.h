...
public:
	...
	bool getNyuForObjId(unsigned short objId, unsigned char& nyuId) const{
		const auto it = s_objIdsToNYUIds.find(objId);
		if (it == s_objIdsToNYUIds.end()) return false;
		nyuId = it->second;
		return true;
	}
...
private:
	...
	void getLabelMappingFromFile(const std::string& filename, const std::string& labelName, const std::string& idName)
	{
		...
		it = header.find("id");
		if (it == header.end()) throw MLIB_EXCEPTION("could not find value id in label mapping file");
		unsigned int objIdx = it->second;
		it = header.find("nyu40id");
		if (it == header.end()) throw MLIB_EXCEPTION("could not find value nyu40id in label mapping file");
		...
		while (std::getline(s, line)) {
			...
			if (!parts[labelIdx].empty() && !parts[idIdx].empty()) {
				unsigned int id = util::convertTo<unsigned int>(parts[idIdx]); // nyuid
				unsigned int obj_id = util::convertTo<unsigned int>(parts[objIdx]);
				...
				s_objIdsToNYUIds[(unsigned short)obj_id] = (unsigned char)id;
			}
		}
		s.close();
	...
	std::unordered_map<unsigned short, unsigned char> s_objIdsToNYUIds;
};